...
class DragClassificationNode(Node):
    def __init__(self):
        super().__init__('drag_classification_node')
    ...
    def _setup_velocity_logging(self):
        log_conf = LogConfig(name='Velocity', period_in_ms=LOG_PERIOD_MS)
    ...
    def _write_log_csv(self):
        if not self.log_rows:
            self.get_logger().warning("No velocity log rows captured, skipping CSV write")
            return
        ...
        if self.sequence == QuadcopterSequence.WAITING_FOR_STATE:
            ...
            if not self._hover_commanded:
                self.cf.high_level_commander.go_to(
                    x=self.starting_position[0], y=self.starting_position[1],
                    z=self.target_z, yaw=0, duration_s=self.HOVER_DURATION / 2
                )
                self._hover_commanded = True
            if self.time_elapsed() > self.HOVER_DURATION:
                self.get_logger().info(f"Commanding impulse: roll={self.IMPULSE_ROLL_DEG} deg, pitch={self.IMPULSE_PITCH_DEG} deg")
                self.enter_state(QuadcopterSequence.IMPULSE)

        elif self.sequence == QuadcopterSequence.IMPULSE:
            self.cf.commander.send_zdistance_setpoint(
                self.IMPULSE_ROLL_DEG, self.IMPULSE_PITCH_DEG, 0, self.target_z
            )
            if self.time_elapsed() > self.IMPULSE_DURATION:
                self.get_logger().info("Impulse complete, coasting...")
                self.enter_state(QuadcopterSequence.COAST)

        elif self.sequence == QuadcopterSequence.COAST:
            self.cf.commander.send_zdistance_setpoint(0, 0, 0, self.target_z)
            if self.time_elapsed() > self.COAST_DURATION:
                self.get_logger().info("Coast complete, returning to start...")
                self.cf.commander.send_notify_setpoint_stop()
                self.cf.high_level_commander.go_to(
                    x=self.starting_position[0], y=self.starting_position[1],
                    z=self.target_z, yaw=0, duration_s=RECOVER_DURATION
                )
            ...
            if self.time_elapsed() > RECOVER_DURATION:
                ...
                self.cf.high_level_commander.land(
                    absolute_height_m=self.starting_position[2], duration_s=LAND_DURATION
                )
...
ros2 run thesis_optitrack_bridge drag_classification_node
'''